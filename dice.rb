...
def dice_prompt
  ...
  if start_answer == "y" && @current_player.wallet > 0
    puts "Place your bet. You have $#{@current_player.wallet} chips left"
    bet = gets.chomp
    while true
      if (@current_player.wallet.to_i - bet.to_i) < 0
    ...
    when 7 || 11
      ...
      @current_player.wallet_balence = @current_player.wallet.to_i + bet.to_i*2
      puts "Your current balence is: #{@current_player.wallet}"
      dice_prompt
      else
        puts "You still won even though you didnt guess the right number"
        puts "You won #{bet.to_i}"
        @current_player.wallet_balence = @current_player.wallet.to_i + bet.to_i
        puts "Your current balence is: #{@current_player.wallet}"
        dice_prompt
      end
    when 2
      ...
      puts "You lost #{bet.*2}"
      @current_player.wallet = @current_player.wallet.to_i - bet.to_i*2
      puts "Your balance is #{@current_player.wallet}"
      dice_prompt
    when 12 || 3
      puts "Sorry about that"
      puts "You lost #{bet}"
      @current_player.wallet = @current_player.wallet.to_i - bet.to_i
      puts "Your balance is #{@current_player.wallet}"
      dice_prompt
    else 4 || 5 || 6 || 8 || 9 || 10
      if guess_for_total.to_i == @d.dice_sum.to_i
